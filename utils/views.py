...
class TranslatableDetailViewMixin(TranslatableSlugMixin, ViewUrlMixin):
    view_url_name = ""
    object = None

    def get_view_url(self):
        with switch_language(self.object, get_language()):
            return reverse(f"{self.view_url_name}", kwargs={"slug": self.object.slug})
...
class CategoriedListView(ListView):
    ...
    def get_queryset(self):
        ...
        if not slug:
            return queryset
        try:
            self.category = self.category_model.objects.active_translations(
                slug=slug
            ).get()
